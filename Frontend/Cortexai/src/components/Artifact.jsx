...
<script> ${jsfile?.content || ""}</script>
  ...
  `
      const handleCopyCode = async () => { // not code
      ...
      {!collapsed ? (
        <div className="flex flex-col h-full bg-[#0d0f14]">
          ...
          <div className="h-14 px-4 border-b border-white/[0.06] flex items-center gap-3 shrink-0">
            <button onClick={() => setCollapsed(true)} className="flex items-center justify-center w-7 h-7 rounded-lg text-slate-500 hover:text-slate-200 hover:bg-white/[0.05] transition-col duration-150 bg-transparent border-none cursor-pointer shrink-0">
              <PanelRightClose size={25} />
            </button>
            <div className="flex items-center gap-2 flex-1 min-w-0">
              <div className="flex items-center justify-center w-6 h-6 rounded-md bg-indigo-500/10 border border-indigo-500/20 shrink-0">
                <Code2 className="text-indigo-400" size={15} />
              </div>
              ...
              <div className="flex items-center gap-1 shrink-0">
                ...
                >
                 {copyCode? <Check size={15}/>:<Copy size={15} />} 
                </button>
              </div>
              ...
              {canPreview && <div className="flex items-center gap-1 bg-white/[0.04] border border-white/[0.06] p-1 rounded-lg">
                ...
                <button onClick={() => setTab("preview")} className={`flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-medium rounded-md transition-colors ${tab === "preview"
                  ? "bg-indigo-500 text-white"
                  : "text-slate-500 hover:text-slate-200"
                  }`}>
                  <Eye size={11} /> Preview
          ...
          {tab==="code" && <div className="flex h-auto border-b border-white/[0.06] overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden shrink-0">
            {artifacts[0]?.files?.map((f, index) => (
              <button key={index}
                ...
                className={`px-4 py-2.5 text-[11px] font-medium whitespace-nowrap transition-colors duration-150 border-r relative cursor-pointer bg-transparent border-white/[0.05] ${activeFile === index
                    ? "text-indigo-400"
  ...
  {tab === "preview" && canPreview ? (
    ...
    >
      ...
      options={{ readOnly: true, minimap: { enabled: false }, fontSize: 13, wordWrap: "on", scrollBeyondLastLine: false, padding: { top: 16 }, lineNumbers: "on", automaticLayout: true, renderLineHighlight: "none" }}
      />
      ...
      ) : <div className="hidden lg:flex h-full border border-white/[0.06] bg-[#0d0f14] flex-col items-center py-4 gap-3 shrink-0">
        <button onClick={() => setCollapsed(false)} className="flex items-center justify-center w-7 h-7 rounded-lg text-slate-500 hover:text-slate-200 hover:bg-white/[0.05] transition-col duration-150 bg-transparent border-none cursor-pointer shrink-0">
